...
$sentenciaExistencia = $base_de_datos->prepare("UPDATE flor_temporada SET existencia = (existencia - ?) WHERE idflortempo = ?;");
foreach ($_SESSION["carrito"] as $producto) {
	$total += $producto->total;
	$sentencia->execute([$producto->idflortempo, $idVenta, $producto->cantidad]);
	$restar = $producto->cantidad;
	$sentenciaExistencia->execute([$restar, $producto->idflortempo]);
}
